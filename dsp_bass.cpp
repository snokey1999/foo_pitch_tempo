...
public:
    ...
    void drain(abort_callback & p_abort) {
        if (m_push_stream && m_fx_stream) {
            ...
            pfc::array_t<audio_sample> accum;
            pfc::array_t<audio_sample> buffer;
            buffer.set_size(m_sample_rate * m_channels);
            ...
            if (accum.get_size() > 0) {
                audio_chunk * out_chunk = insert_chunk(accum.get_size() / m_channels);
                out_chunk->set_data(accum.get_ptr(), accum.get_size() / m_channels, m_channels, m_sample_rate);
            }
        }
    }
...
public:
    ...
    bool on_chunk(audio_chunk * chunk, abort_callback & p_abort) override {
        ...
        if (m_push_stream) {
            BASS_StreamPutData(m_push_stream, data, (DWORD)(sample_count * channels * sizeof(audio_sample)));
            ...
            while (true) {
                DWORD bytes_read = BASS_ChannelGetData(m_push_stream, buffer.get_ptr(), (DWORD)(buffer.get_size() * sizeof(audio_sample)));
                if (bytes_read == 0 || bytes_read == (DWORD)-1) break;
